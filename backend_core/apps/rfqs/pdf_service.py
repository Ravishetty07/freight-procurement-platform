...
def generate_contract_pdf(bid):
    ...
    # Calculate a mock percentage for the visual cost bar (assuming 80% base, 20% surcharges for UI purposes)
    # You can replace these with actual dynamic calculations if you have the data
    base_cost_pct = 80
    surcharge_pct = 20
    ...
    <head>
        ...
        <style>
            ...
            /* --- HEADER & LOGO --- */
            .header-table {{ width: 100%; border-bottom: 3px solid #0f172a; padding-bottom: 20px; margin-bottom: 25px; }}
            .logo {{ font-size: 32px; font-weight: 900; color: #0f172a; margin: 0; letter-spacing: -1.5px; }}
            ...
            .doc-date {{ font-size: 12px; color: #2563eb; text-align: right; font-weight: bold; margin-top: 5px; }}
            
            /* --- TWO COLUMN PARTIES --- */
            .parties-table {{ width: 100%; margin-bottom: 30px; }}
            .party-box {{ background-color: #f8fafc; padding: 18px; border-left: 4px solid #cbd5e1; }}
            .party-box.vendor {{ border-left: 4px solid #2563eb; background-color: #eff6ff; }}
            .party-spacer {{ width: 4%; }}
            .party-title {{ font-size: 9px; color: #64748b; text-transform: uppercase; margin-bottom: 8px; font-weight: bold; letter-spacing: 0.5px; }}
            ...
            .metrics-table {{ width: 100%; margin-bottom: 25px; background-color: #ffffff; }}
            .metric-box {{ text-align: center; padding: 15px 10px; border: 1px solid #e2e8f0; width: 33%; }}
            .metric-label {{ font-size: 9px; color: #64748b; text-transform: uppercase; font-weight: bold; letter-spacing: 0.5px; }}
            .metric-value {{ font-size: 22px; font-weight: bold; color: #2563eb; margin-top: 6px; }}
            ...
            .bar-base {{ background-color: #1e293b; height: 8px; }}
            .bar-surcharge {{ background-color: #3b82f6; height: 8px; }}
            ...
            .financials-table th {{ background-color: #f1f5f9; color: #475569; padding: 12px 15px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; }}
            .financials-table td {{ padding: 12px 15px; border-bottom: 1px solid #e2e8f0; font-size: 12px; color: #0f172a; }}
            .total-row td {{ background-color: #f8fafc; border-bottom: none; font-size: 20px; color: #0f172a; text-align: right; font-weight: bold; padding: 18px 15px; }}
            .total-label {{ text-align: left !important; font-size: 14px !important; color: #64748b !important; text-transform: uppercase; }}
    ...
    <body>
        ...
        <table class="parties-table">
            <tr>
                ...
                <td class="party-box vendor" style="width: 48%;">
                    <div class="party-title">Carrier / Executing Party</div>
                    <div class="party-name">{vendor.company_name or vendor.username}</div>
        ...
        <table class="metrics-table" cellspacing="8">
            <tr>
                <td class="metric-box">
                ...
                <td class="metric-box" style="background-color: #f8fafc;">
                    <div class="metric-label">Cargo Volume</div>
                    <div class="metric-value" style="color: #0f172a;">{bid.shipment.volume}x</div>
        ...
        <table class="timeline-table">
            <tr>
                ...
                <td class="timeline-line">
                    <span style="font-size: 10px; color: #2563eb; background: #fff; padding: 0 10px; font-weight: bold;">OCEAN TRANSIT</span>
                </td>
                <td class="timeline-node">
